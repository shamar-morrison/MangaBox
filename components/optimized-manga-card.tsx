...
export function OptimizedMangaCard({ manga }: OptimizedMangaCardProps) {
  ...
  // Use the dedicated cover API route for better caching
  const coverUrl = coverArt?.attributes?.fileName
    ? `/api/manga-cover/${manga.id}/${coverArt.attributes.fileName}`
    : "/placeholder.svg?height=300&width=200"

  return (
    <Link href={`/manga/${manga.id}`}>
      <Card className="group cursor-pointer transition-all duration-300 hover:scale-105 bg-gray-900/50 border-gray-800 hover:border-purple-500 overflow-hidden">
